...
/* jshint -W098 */
angular.module('mean.stacks').controller('LogsController', ['$scope', 'Global', 'Appsun', '$stateParams', 'MeanSocket', '$timeout', '$anchorScroll', 'Platforms', 'Articles',
    function($scope, Global, Appsun, $stateParams, MeanSocket, $timeout, $anchorScroll, Platforms, Articles) {
        ...
        vm.find = function() {
            ...
            Appsun.logs(options, function(data) {
                ...
                if (options.from === 0) {
                    $timeout(function() {
                        divLogs = $('#logsContent');
                        if (divLogs) divLogs.scrollTop($('#logsContent')[0].scrollHeight);
                    }, 0);
                } else scrollData('set');
            });
        };
        ...
        function scrollData(action) {
            if (action === 'save') {
                prevScroll = divLogs.scrollTop();
                prevHeight = divLogs[0].scrollHeight;
            } else if (action === 'set') {
                $timeout(function() {
                    divLogs.scrollTop(prevScroll + divLogs[0].scrollHeight - prevHeight);
                }, 100)
            }
        }
