...
/* jshint -W098 */
angular.module('mean.stacks').controller('SessionsController', ['$scope', 'Global', 'Stacks', 'Appsun', '$stateParams', 'MeanSocket', '$timeout', '$state', '$rootScope', '$location',
    function($scope, Global, Stacks, Appsun, $stateParams, MeanSocket, $timeout, $state, $rootScope, $location) {
        ...
        vm.goTo = function(to, session) {
            $location.url('/stacks/' + $stateParams.id + '/' + to);
