...
angular.module('mean.apps').factory('Stacksun', ['Sapi', '$q', '$http', '$location',
    function(Sapi, $q, $http, $location) {
        ...
        function appSunData(options, cb) {
            ...
            Sapi.get({
                cmd_api: '/sapi/stacksun/search/' + options.index + '/' + options.type + '?size=' + options.size + '&from=' + options.from + '&range=' + options.range + '&filters=' + query
            }).then(function(docs) {
