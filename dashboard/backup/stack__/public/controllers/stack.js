...
/* jshint -W098 */
angular.module('mean.stack').controller('StackController', ['$scope', 'Global', 'Stack', '$stateParams', '$state', '$uibModal', '$rootScope', '$timeout', 'MeanSocket', 'ConfigApp', 'Integrations', 'localStorageService', 'Tags', '_',
  function($scope, Global, Stack, $stateParams, $state, $uibModal, $rootScope, $timeout, MeanSocket, ConfigApp, Integrations, localStorageService, Tags, _) {
    $scope.global = Global;
    $scope.$state = $state;
    ...
    $scope.search_tags = [];
    $scope.show_taginput = false;

    $scope.showInputTags = function(){
      $scope.show_taginput = true;
    }
    ...
    $scope.localSearch = function(str, scope) {
      ...
      var promise = new Promise(function(resolve, reject) {
        Tags.find($scope.stack.company, str, function(err, tags) {
          vms.search_tags = [];
          if(!err){
            var exist_in_array = false;
            _.each(tags, function(value){
              vms.search_tags.push({value: value.value, tag: value});
              if(value.value == str){
                exist_in_array = true;
              }
            });
            ...
            if(exist_in_array == false) {
              vms.search_tags.unshift({value: str, desc: 'new', tag: {}});
            }
