(function (factory) {
    ...
    Chart.types.Line.extend({
        ...
        draw: function () {
            ...
            if(this.options.redline){
                var point = this.datasets[0].points[0]
                var scale = this.scale
                var per_one_point = (scale.endPoint - scale.startPoint) /  scale.max;
                for(var i = 0; i < this.chart.width; i = i + 20){
                    if(i === 0){
                        var start = this.datasets[0].x;
                    } else{
                        var start = i;
                    }
                    var call_y = ((scale.max - this.options.redline) * per_one_point) + scale.startPoint;
                    this.chart.ctx.beginPath();
                    this.chart.ctx.moveTo(start, call_y);
                    this.chart.ctx.strokeStyle = '#ff0000';
                    this.chart.ctx.lineTo((start + 10), call_y);
    ...
    /*
    function exnendLWL(tscope){
        var local_scope = tscope;
    ...
    */
    ...
    return angular.module('chart.js', [])
        ...
        .directive('chartLineWithLine', ['ChartJsFactory', function (ChartJsFactory) { return new ChartJsFactory('LineWithLine'); }])
   ...
   * })))
    ...
    function ChartJsFactory (ChartJs, $timeout) {
        return function chart (type) {
            return {
                ...
                link: function (scope, elem/*, attrs */) {
                    ...
                    ['data', 'labels', 'redline', 'options', 'series', 'colours', 'legend', 'click', 'hover'].forEach(deprecated);
                    ...
                    scope.$watch('redline', resetChart, true);
                    ...
                    function createChart (type) {
                        ...
                        var data = Array.isArray(scope.data[0]) ?
                            getDataSets(scope.labels, scope.redline, scope.data, scope.series || [], scope.colours) :
                        ...
                        var options = angular.extend({}, ChartJs.getOptions(type), scope.options, {redline: scope.redline});
                        chart = new ChartJs.Chart(ctx)[type](data, options);
                        //if(type == 'LineWithLine'){
                        //    exnendLWL(scope);
                        //}
            ...
            return function (evt) {
                ...
                if (atEvent) {
                    ...
                    if (triggerOnlyOnChange === false || angular.equals(lastState, activePoints) === false) {
                        ...
                        scope[action](activePoints, chart, evt);
                        scope.$apply();
                    }
                }
            };
        ...
        function getDataSets (labels, redline, data, series, colours) {
            return {
                labels: labels,
                redline: redline,
