...
/* jshint -W098 */
angular.module('mean.stack').controller('IntegrationsController', ['$scope', 'Global', '$stateParams', 'MeanSocket', 'Integrations', 'Stack', '$state', '_', 'ConfigApp', 'slideMenu',
    function($scope, Global, $stateParams, MeanSocket, Integrations, Stack, $state, _, ConfigApp, slideMenu) {
        ...
        ConfigApp.get().then(function(config) {
            vmi.gitlabCmd = config.apiHost + '/integrations/webhook/gitlab?type=event&token=' + vmi.global.user.profile.token + '&appId=' + $scope.stack._id;
        });
        ...
        vmi.update = function() {
            Stack.update($scope.stack._id, $scope.stack, function(err, data) {
        ...
        vmi.slideIsOpen = false;

        vmi.triggerSlideMenuPopover = function(service, menu_id) {
            if(service && service.settings){
                $stateParams.service = service.name;
                vmi.findGlobalIntegration();
                vmi.findIntegration(service.name);
                slideMenu.slide(angular.element("#slide-menu-integration"), service.name);
                vmi.slideIsOpen = !vmi.slideIsOpen;
            } else{
                if(vmi.slideIsOpen == true){
                    slideMenu.slide(angular.element("#slide-menu-integration"), service);
                    vmi.slideIsOpen = !vmi.slideIsOpen;
                }
            }
        }
