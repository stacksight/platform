...
/* jshint -W098 */
angular.module('mean.stack').controller('IntegrationsPlatformController', ['$scope', 'Global', 'Stacks', 'Appsun', '$stateParams', 'MeanSocket', 'ConfigApp',
    function($scope, Global, Stacks, Appsun, $stateParams, MeanSocket, ConfigApp) {
// /integrations/webhook/platform?type=event&token=ss-dbab64576c69701b97835329c7de36e929d1c8e4ec79827f32e3e227e20c77a9
        var vmip = this;
        vmip.global = Global;
        ConfigApp.get().then(function(config) {
            vmip.cmd = 'platform integration:add --type webhook --url "' + config.apiHost + '/integrations/webhook/platform?type=event&token=' + vmip.global.user.profile.token + '"';
        });
        ...
        vmip.types = {
            pushed: {
                title: 'Pushed',
                name: 'pushed',
                ico_img: '/stack/assets/img/arrow_right.png',
                color: '#0e6b93',
                enabled: true
            },
            branched: {
                title: 'Branched',
                name: 'branched',
                ico_img: '/stack/assets/img/arrow_top.png',
                color: '#e89300',
                enabled: true
            }
        };
