...
/* jshint -W098 */
angular.module('mean.stack').controller('GroupDashboardController', ['$scope', '$filter', '$state', '$location', '$timeout', '$stateParams', '_', 'Health', 'Apps', '$rootScope', 'Stack', 'Stacks', 'Global', '$window', 'Appsun',
    function($scope, $filter, $state, $location, $timeout, $stateParams, _, Health, Apps, $rootScope, Stack, Stacks, Global, $window, Appsun) {
        var vmd = this,
            threshold = {},
        ...
        $window.dynamicEventHeight = 134;

        vmd.readyToShow = false;

        var insOptions = {};
        insOptions.id = appId;
        insOptions.index = 'health';
        insOptions.type = 'security,performance,ssl_validity';
        insOptions.from = 0;
        insOptions.size = 1;
        ...
        Stacks.find(options, function(err, data) {
            vmd.readyToShow = true;
            if (err) {
                return console.log('find stacks error', err);
            } else {
                vmd.allStacks = data;
                Appsun.aggregation('health', insOptions, function(data) {
                    ...
                    data.forEach(function(d) {
                        ...
                        _.map(widgets_input, function(widget) {
                            var chart = {};
                            switch (widget.type) {
                                case 'pointslist':
                                    vmd.insights.pointslist = widget.pointslist;
                                    break;
                                case 'bool':
                                    vmd.insights.bool[widget.title] = widget;
                                    break;
                                default:
                                    break;
                            }
        ...
        Stack.avgScore(appId).then(function(data) {
            // $scope.policy = $scope.vmd.app.policy;
            $scope.policy = 50;
        ...
        vmd.initType = function(type) {
            ...
            // $scope.vmd.app.policy = $scope.vmd.app.policy || {};
            // obj[type] = ($scope.vmd.app.policy[type] && $scope.vmd.app.policy[type].value) ? $scope.vmd.app.policy[type].value : vmd.defaultPolicy.value;
            ...
            //         var initialVal = ($scope.vmd.app.policy && $scope.vmd.app.policy[vmd.currentType]) ? $scope.vmd.app.policy[vmd.currentType].value : vmd.defaultPolicy.value;
