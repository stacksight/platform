...
/* jshint -W098 */
angular.module('mean.billing').controller('BillingController', ['$scope', 'config', 'Global', 'Billing', 'StripeService', '$stateParams', '_', '$timeout', '$state', 'Profile', 'Companies',
    function($scope, config, Global, Billing, StripeService, $stateParams, _, $timeout, $state, Profile, Companies) {
        var vmb = this;
        ...
        vmb.company = Companies.my(vmb.global.user.companies).name;

        function pay(plan, coupon) {

            Billing.pay(plan, coupon, vmb.company, function(err, data) {
        ...
        vmb.getPlans = function(planId) {
            Billing.plans(planId, function(err, data) {
                if (err) console.log(err);
                if(!vmb.plans) vmb.plans = data.plans;
        ...
        vmb.subscribe = function(plan) {
            vmb.payClicked = true;
            vmb.planToUpgrade = plan;
