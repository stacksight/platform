...
angular.module('mean.billing').factory('Billing', ['Sapi', 'ConfigApp', '_', 'Global', 'Companies',
    function(Sapi, ConfigApp, _, Global, Companies) {
        var features;
        var plansData;
        ...
        return {
            plans: function(id, cb) {
                ...
                Sapi.get({
                    //cmd_api: '/sapi/plans/company/' + Global.user.companies[0]._id
                    cmd_api: '/sapi/plans/personal'
            ...
            pay: function(plan, coupon, company, cb) {
                var data = { plan: plan._id };
                if (coupon) data.coupon = coupon._id;
                if (!company) return cb('Company name is required!');
                data.companyName = company;
                Sapi.post({
                    cmd_api: '/subscriptions/' + Companies.my(Global.user.companies).id,
                    data: data
