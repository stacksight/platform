...
angular.module('mean.platforms').directive('embedCode', ['Global', function(Global) {
    return {
        ...
        link: function(scope, element, attrs) {
            ...
            scope.data = {};
            scope.data.platform = scope.stack.platform;
            ...
            scope.text.drupal6 += '    define(\'STACKSIGHT_TOKEN\', \'' + scope.stack.company.owner.token + '\');\n'
            scope.text.drupal6 += '    define(\'STACKSIGHT_APP_ID\', \'' + scope.stack._id + '\');\n';
            scope.text.drupal6 += '    require_once($ss_inc);\n';
            scope.text.drupal6 += '}\n'
            scope.text.drupal6 += '// StackSight end config';

            scope.text.drupal7 = '// StackSight start config\n';
            scope.text.drupal7 += '$ss_inc = DRUPAL_ROOT . \'/\' . dirname(drupal_get_filename(\'module\', \'stacksight\')) . \'/stacksight-php-sdk/bootstrap-drupal-7.php\';\n';
            scope.text.drupal7 += 'if(is_file($ss_inc)) {\n';
            scope.text.drupal7 += '    define(\'STACKSIGHT_TOKEN\', \'' + scope.stack.company.owner.token + '\');\n'
            scope.text.drupal7 += '    define(\'STACKSIGHT_APP_ID\', \'' + scope.stack._id + '\');\n';
            scope.text.drupal7 += '    require_once($ss_inc);\n';
            scope.text.drupal7 += '}\n'
            scope.text.drupal7 += '// StackSight end config';

            scope.text.drupal8 = '// StackSight start config\n';
            scope.text.drupal8 += '$ss_inc = DRUPAL_ROOT . \'/modules/stacksight/stacksight-php-sdk/bootstrap-drupal-8.php\';\n';
            scope.text.drupal8 += 'if(is_file($ss_inc)) {\n';
            scope.text.drupal8 += '    define(\'STACKSIGHT_TOKEN\', \'' + scope.stack.company.owner.token + '\');\n'
            scope.text.drupal8 += '    define(\'STACKSIGHT_APP_ID\', \'' + scope.stack._id + '\');\n';
