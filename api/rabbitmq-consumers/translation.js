...
var producer = require('../producers/index'),
    translationIndexes = ['health', 'integrations-platform'],
    _ = require('lodash'),
    Global = require('../services/global'), //integration-node_ssl_validity-
...
function handleMessage(message, error, done) {
    ...
    for (var i = 0; i < translationIndexes.length; i++) {
        var index = translationIndexes[i];
        if (message.index.indexOf(index) !== -1) indexKey = index;
    }

    if (!indexKey || !translations[indexKey]) {
        done();
        return producer.createJob('identify-data-sts', message);
    }

    try {
        translations[indexKey](message, function(messages) {
            // messages.forEach(function(_message) {
            //     producer.createJob('identify-data-sts', _message);
            // });
        });
        done();
    } catch (e) {
...
translations['integrations-platform'] = function(message, cb) {
    ...
    producer.createJob('identify-data-sts', message);
};
...
function loop(messages, message, count, data, cb) {
    if (_.isArray(data)) {
        ...
        var key = category;
        if (!healthTranslation[key]) key = '_undefined';

        healthTranslation[key](data, message, function(_o) {
