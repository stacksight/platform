...
q.on('message', function(message) {
    ...
    async.parallel({
            elastic: function(callback) {
                ...
                if (!stackIds.length) return callback('NO STACKS TO ' + message.groupId + ' GROUP');
                ...
                Elastic.search(options, function(error, response) {
                    if (error) {
                        console.error('ELASTIC SEARCH HEALTH AGGREGATION ERROR: ', error.status + ' ' + error.displayName);
        ...
        function(err, results) {
            ...
            if (err) return console.error('FINAL PARALLEL FUNCTION ERR', err);


            if (results.elastic.error || !results.elastic.data || !results.elastic.data.dates.buckets.length) return console.log('-------- APP AGGREGATION - NO DATA --------------');
